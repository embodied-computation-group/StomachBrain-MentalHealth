function [X1,Y1,Z1] = CCAprepAllData_HeartBrain()

% prepare HeartBrain data, psych data, and nuisance vars for CCA
...
ImagingFile = 'HFhrv_difumo256_mriqc.csv'; %LFhrv_difumo256_mriqc.csv'  Resprv_difumo256_mriqc';  % HeartBrain (- difumo parcellated) 
...
HeartBrain_filepath     = fullfile(data_dir, ImagingFile);
HeartBrain_file      = readtable(HeartBrain_filepath);
% sort based on subID
HeartBrain_file = sortrows(HeartBrain_file, 1);
...
HeartBrain_file(:,idx2rem+1) = []; % + 1 due to sub_ID col

% convert to double
HeartBrain_file       = table2array(HeartBrain_file);
...
%%%%%%%% control HeartBrain analysis use same subIDs as StomachBrain %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % load subIDs from gastricPLV analysis
usableSubs = load([data_dir,'CCA_input/subIDs_CCA_PsychScoresSubscales.mat']).subIDs; 
Y0                  = HeartBrain_file(ismember(HeartBrain_file(:,1),usableSubs),:); %netMats(ismember(netMats(:,1),usableSubs),:);
X0                  = Beh_file(ismember(Beh_file(:,1),usableSubs),:); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%setdiff(X0(:,1), Y0(:,1)); % some participants don't have heartbrain due
%to noisy segments of cardiac data where IBIs not able to be computed (need
%continuous R-peak times for duration of scan)
...
Y0                  = HeartBrain_file(ismember(HeartBrain_file(:,1),usableSubs),:);
X0                  = Beh_file(ismember(Beh_file(:,1),usableSubs),:); 
...
if saveOutput
    filename    = 'cca_inputs_difumo256_HFhrvHeartBrain_PsychScoresSubscales';
