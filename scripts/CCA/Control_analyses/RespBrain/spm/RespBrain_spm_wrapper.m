...
% master wrapper for individual resp-brain maps
...
for n = 1 : length(fmri_files)
    ...
    regress_path = sprintf('/mnt/raid0/scratch/BIDS/derivatives/physio/resp/rest/Respmotionacompcor_regressors/%s_regressors_Resp6motion6acompcor.mat', subID);
    ...
    regress_path_aligned = sprintf('/mnt/raid0/scratch/BIDS/derivatives/physio/resp/rest/Respmotionacompcor_regressors/%s_regressors_Resp6motion6acompcor_noNaN.mat', subID);
    save(regress_path_aligned, 'R', 'names');
    
    % save path of heart-brain coupling each subject
    savepath = sprintf('/mnt/raid0/scratch/BIDS/derivatives/resp-brain_coupling/%s/',subID);
    if exist(savepath, 'dir') == 0
        mkdir(savepath)
    end
    
    % run SPM batch
    RespBrain_spm(savepath, frame_names, regress_path_aligned);
