%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% SCRIPT_EGG_main - edited by Leah Banellis Feb 2023 to compute EGG measures
%%% 
...
for subj = 1:length(subj_id)

    %sub-0144 (subj = 112) sub-0431 (subj=238) sub-0443 (subj=248) - rejected
    subj_name = subj_id(subj);
    if subj_name == 144 || subj_name == 431 || subj_name == 443
        continue
    end
    ...
    [FFT_EGG] = compute_FFT_EGG_LB(EGG_raw, 1);

    %saveas(gcf, sprintf('/Users/au704655/Documents/EGG/Figures/SpectraPlots_EachRange/%.4d_GastriaSpectraRanges.png', subj_name))
   ...
   % filtered EGG, using the Hilbert method.
    ...
    % bradygastria
    if ~isnan(FFT_EGG.max_freq_brady)I
