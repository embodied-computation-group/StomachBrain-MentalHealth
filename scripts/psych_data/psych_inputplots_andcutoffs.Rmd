...
filepath = '~/Git/StomachBrain-MentalHealth/data/CCAinput/cca_inputs_difumo256_gastricPLV_PsychScoresSubscales.mat'  #'~/Documents/StomachBrain/CCA/data/CCA_input/cca_inputs_difumo256_gastricPLV_PsychScoresSubscales.mat'
...
psych <- read.table('~/Git/StomachBrain-MentalHealth/data/psych_scores/grand_surveyscores_summary.tsv', header = TRUE, sep = "\t") #~/Documents/StomachBrain/CCA/data/grand_surveyscores_summary.tsv', header = TRUE, sep = "\t")
subIDs = readMat('~/Git/StomachBrain-MentalHealth/data/CCAinput/ControlCCAs/subIDs_CCA_PsychScoresSubscales.mat')$subIDs
  #'~/Documents/StomachBrain/CCA/data/CCA_input/subIDs_CCA_PsychScoresSubscales.mat')$subIDs
...
demograph <- read.table('/mnt/raid0/scratch/BIDS/phenotype/demographics/demographics.tsv', header = TRUE, sep = "\t") 
subIDs = readMat('~/Git/StomachBrain-MentalHealth/data/CCAinput/ControlCCAs/subIDs_CCA_PsychScoresSubscales.mat')$subIDs
# select subjects in CCA 
demograph <- demograph %>% filter(as.numeric(str_sub(participant_id, -4)) %in% subIDs[, 1])


# Plot with histogram + rug
plotage <- ggplot(demograph, aes(x = age)) +
  geom_histogram(
  ...
  geom_jitter(aes(y = 0), width = 0.3, height = 0, alpha = 0.2, shape = '|', size = 4, color = "skyblue4") +
  #geom_rug(sides = "b", alpha = 0.2, linewidth = 1) +  # rug plot at bottom 
  labs(
...
demograph$gender <- factor(demograph$gender, levels = c(1, 2), labels = c("Female", "Male"))
plotgender <- ggplot(demograph, aes(x = gender)) +
  geom_bar(
...
ggsave("~/Git/StomachBrain-MentalHealth/figures/methods/demograph/age_gender_distr_rug.png", combined_plot, width = 12, height = 6)
