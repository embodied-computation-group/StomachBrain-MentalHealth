...
% VMP1 A/B/C montage all 6 bipolar EGG channels (2 diff positioning & 2 diff size of electrodes)
% 71 B/2 ECG Montage
% 117 C EGG Montage
% 381 vmp2 (3 electrodes = 1 bipolar EGG channel & ref)
% 433 vmp2 (7 electrodes = 3 bipolar EGG channel & ref)

root_data_path='/Volumes/Seagate/Physio_VMP/data/'
subj_name= 60


%% Load data
if subj_name > 256 %for vmp2 CHECK!!
...
RawTimeSeriesFilename = sprintf('%.4d_RawTimeSeriesPlot',subj_name);
...
%     else
    ...
    SpectrumFilename = sprintf('%.4d_spectrumPlot',subj_name);
    if cfgMain.savePlots == 1
...
%     plot(EGGTimeseries)
    %% Fill log file
    EGGPhaseXVolumeFilename = sprintf('%.4d_EGGPhaseXVolume',subj_name);
    EGGAmplitudeXVolumeFilename = sprintf('%.4d_AmplitudeXVolume',subj_name);
...
%     
    ...
    filteredPlotFilename = sprintf('%.4d_filteredPlot',subj_name);
    if cfgMain.savePlots == 1
